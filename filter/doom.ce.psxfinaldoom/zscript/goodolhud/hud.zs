class GoodOlHUDStatusBar : PsxStatusBar
{
    HUDFont GOHmHUDFont;
    ...
    override void Draw (int state, double TicFrac)
    {
        ...
        if (state == HUD_StatusBar)
        {
            if (hud_weapons_original)
            {
                ...
                if (screenblocks == 10 && previousScreenblocks != screenblocks) { UpdateScreenGeometry(); }
            }
            else { BeginStatusBar(); }
    ...
    protected void GOHDrawFullScreenStuff ()
    {
        ...
        if (CVar.GetCVar("hud_mugshot_original", players[consoleplayer]).GetBool()) { playerMugshot = "PSX"; }
        else if (CPlayer.GetGender() == 1) { playerMugshot = "GAL"; }

        let canshowmugshot = CVar.FindCVar("goh_showmugshot").GetBool() && GetMugShot(5, MugShot.XDEATHFACE|MugShot.CUSTOM, playerMugshot).IsValid(); // currently hides when the xdeath animation finishes
    ...
    protected virtual void GOHDrawMugShot(int coordbasex, int coordbasey)
    {
        ...
        DrawTexture(GetMugShot(5, MugShot.XDEATHFACE|MugShot.CUSTOM, playerMugshot), (coordbasex + 19.6, coordbasey + 17.5), DI_SCREEN_LEFT_BOTTOM|DI_ITEM_CENTER, GOHHideOnDeath(1., PlayerDeadTimerCheckMugshot), (35, 31));
    }
