class GoodOlHUDStatusBar : StrifeStatusBar
{
    HUDFont GOHmHUDFont;
    ...
    protected virtual void GOHDrawMugShot(int coordbasex, int coordbasey)
    {
        ...
        DrawTexture(GetMugShot(5), (coordbasex + 19.6, coordbasey + 17.5), DI_SCREEN_LEFT_BOTTOM|DI_ITEM_CENTER, GOHHideOnDeath(1., PlayerDeadTimerCheckMugshot), (35, 31));
    }
    ...
    protected virtual void GOHDrawPowerupTimers(int coordbasex, int coordbasey)
    {
        ...
        for (checkedpowerups = 0; checkedpowerups < maxpowerups; checkedpowerups += 3)
        {
            ...
            switch (PowerupDefinitions[checkedpowerups])
            {
              ...
              case 'PowerInvulnerable': // respawn protection check since this iwad has no invuln powerup
                if (!currentpowerup || !CPlayer.mo.bRespawnInvul) { continue; }
                break;
    ...
    {
        int maxammocapacities = 7 * 2;

        static const Name AmmoCapacityDefinitions[] =
        {
            "ClipOfBullets",           "[Yellow]",
            "PoisonBolts",             "[Green]",
            "ElectricBolts",           "[Blue]",
            "HEGrenadeRounds",         "[Orange]",
            "PhosphorusGrenadeRounds", "[Fire]",
            "MiniMissiles",            "[Brown]",
            "EnergyPod",               "[LightBlue]"
        };
